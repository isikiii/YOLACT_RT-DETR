...
class RTDETRTransformer(nn.Module):
    ...
    def forward(self, feats, targets=None):
        ...
        if self.training and self.num_denoising > 0:
            ...
            denoising_class, denoising_bbox_unact, attn_mask, dn_meta = (None,)*4

        # --------------------------------------------------------------
        # 3. 取 encoder top-k 结果，生成 decoder 初始 query
        # --------------------------------------------------------------
        (tgt, init_ref_points_unact,
         enc_topk_bboxes, enc_topk_logits) = self._get_decoder_input(
            memory, spatial_shapes, denoising_class, denoising_bbox_unact)

        # --------------------------------------------------------------
        # 4. Decoder：一次性得到各层 bbox / logit / mask
        # --------------------------------------------------------------
        out_bboxes, out_logits, out_masks = self.decoder(
            ...
            self.dec_bbox_head,
            self.dec_score_head,
            self.dec_mask_head,          # ★ 新增
            self.query_pos_head,
            attn_mask=attn_mask)

        # --------------------------------------------------------------
        # 5. 如果启用了 denoising，则把 DN & 主干部分剥离
        # --------------------------------------------------------------
        if self.training and dn_meta is not None:
            # 按照 dn_num_split=[dn_len, normal_len] 在 dim=2 拆分
        ...
        # --------------------------------------------------------------
        out = {
            'pred_logits': out_logits[-1],   # [B,Q,C]
            'pred_boxes' : out_bboxes[-1],   # [B,Q,4]
            'mask_coefs' : out_masks[-1]     # [B,Q,k]  ★
        }

        # --------------------------------------------------------------
        # 7. 训练阶段：aux-loss & denoising 额外输出
        # --------------------------------------------------------------
        if self.training and self.aux_loss:
            out['aux_outputs'] = self._set_aux_loss(
                out_logits[:-1], out_bboxes[:-1], out_masks[:-1])
            # 把 encoder top-k 也加入 aux（没有 mask_coefs，用 None 占位）
            out['aux_outputs'].extend(
                self._set_aux_loss([enc_topk_logits],
                                   [enc_topk_bboxes],
                                   [None]))
            ...
            if dn_meta is not None:
                out['dn_aux_outputs'] = self._set_aux_loss(
                    dn_out_logits, dn_out_bboxes, dn_out_masks)
                out['dn_meta'] = dn_meta
    ...
    @torch.jit.unused
    # 接收 mask 列表，多一个参数；若某层无 mask（如 enc_topk），传 None
    def _set_aux_loss(self, outputs_class, outputs_coord, outputs_mask):
        aux = []
        for cls_, box_, msk_ in zip(outputs_class, outputs_coord, outputs_mask):
            d = {'pred_logits': cls_, 'pred_boxes': box_}
            if msk_ is not None:
                d['mask_coefs'] = msk_
            aux.append(d)
        return aux
